...
module.exports = {
  ...
  description: 'Start a simon says game',
   async execute(client, message) {
    if(!permissions.has(Permissions.FLAGS.MANAGE_ROLES) || !permissons.has(Permissions.FLAGS.MANAGE_CHANNELS)){
      return message.channel.send(`I do not have permissions to run the game. I require the MANAGE MESSAGES, MANAGE CHANNELS, MANAGE ROLES, and MANAGE ROLES permissions to do so.`)
    }
    ...
    .then(async (c)=>{
      ...
      e.setDescription(`A new simon says game has started! Give the people that want to play the <@&${roles.player.id}> role or use the \`playing\` command. Use the \`y!help says\` command to see a list of available commands.`)
